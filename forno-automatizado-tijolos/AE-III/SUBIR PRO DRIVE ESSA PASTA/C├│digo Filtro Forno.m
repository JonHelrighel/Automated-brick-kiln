...
% Equivalente ao loop acima usando a função filter
N = 20;
b = [1/N];
a = [1, -(1 - 1/N)];
sig_filtrado = filter(b, a, sig_crop);

plot(t_crop, sig_crop);
hold on;
plot(t_crop, sig_filtrado);
figure();
...
plot(t_crop, sig_filtrado);
figure();
pwelch(sinalFiltrado,[],[],[],1);
figure();
pwelch(sig_filtrado,[],[],[],1);
